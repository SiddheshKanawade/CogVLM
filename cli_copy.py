...
def main():
    ...
    parser.add_argument(
        "--version", type=str, default="chat", help="version to interact with"
    )
    parser.add_argument(
        "--from_pretrained", type=str, default="cogvlm-chat", help="pretrained ckpt"
    )
    parser.add_argument(
        "--local_tokenizer",
        type=str,
    ...
    with torch.no_grad():
        while True:
            ...
            for query in questions:
                ...
                ## Individual query start
                if query == "clear":
                    break
                if query == "stop":
                    sys.exit(0)
                try:
                    print(f"Getting Answer from the model for {query}")
